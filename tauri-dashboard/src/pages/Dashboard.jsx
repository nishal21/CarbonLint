...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        async function fetchLive() {
            if (!mounted) return;
            try {
                const [stats, profilingStatus] = await Promise.all([
                    api.getCurrentStats(),
                    api.getProfilingStatus().catch(() => ({ active: false })),
                ]);
                if (mounted) {
                    setLiveStats(stats);
                    // Sync profiling status with tray
                    setProfiling(profilingStatus);
                }
            } catch (e) { }
        }
    ...
    const handleStartProfiling = async () => {
        try {
            ...
            showToast('⚡ Profiling started! Recording system metrics...');
        } catch (err) {
    ...
    const handleStopProfiling = async () => {
        try {
            ...
            setRuns(runsData);
            setSummary(summaryData);
            showToast(`✅ Recorded! Carbon: ${result.carbon.total_grams.toFixed(4)} gCO2 | Duration: ${result.resources.wall_time.toFixed(1)}s`);
        } catch (err) {
                ...
                <div className="card mb-lg" style={{ borderColor: '#22C55E', borderWidth: '1px' }}>
                    <div className="flex gap-xl items-center flex-wrap">
                        ...
                        <div className="flex items-center gap-xs">
                            ...
                            <strong>{liveStats.cpu?.utilization?.toFixed(1) || 0}%</strong>
                        </div>
                        <div className="flex items-center gap-xs">
                            <HardDrive size={16} className="text-secondary" />
                            <span className="text-secondary">Memory:</span>
                            <strong>{liveStats.memory?.usage_percent || 0}%</strong>
                        </div>
                        {liveStats.disk && (
                            <div className="flex items-center gap-xs">
                                <BarChart3 size={16} style={{ color: '#F97316' }} />
                                <span className="text-secondary">Disk:</span>
                                <strong>{(liveStats.disk.read_per_sec / 1024).toFixed(1)} KB/s</strong>
                            </div>
                        )}
                        {liveStats.network && (
                            <div className="flex items-center gap-xs">
                                ...
                                <strong>{(liveStats.network.rx_per_sec / 1024).toFixed(1)} KB/s</strong>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {runs.slice(0, 5).map((run) => {
                                    const { color, level } = api.getImpactLevel(run.carbon?.total_grams || 0);
                                    ...
                                    return (
                                        <tr key={run.id}>
                                            ...
                                            <td><span className={`badge badge-${color}`}>{level}</span></td>
