                            ...
                            <div className="flex flex-col gap-lg">
                                ...
                                <div className="flex justify-between items-center">
                                    <div>
                                        ...
                                        <div className="text-secondary text-sm">Launch CarbonLint automatically when you log in</div>
                                    </div>
                                    <Toggle checked={settings.autoStart || false} onChange={async (v) => {
                                        updateSetting('autoStart', v);
                                        await api.enableAutostart(v);
                                    }} />
                                </div>
                                <div>
                                    <label className="text-secondary text-sm mb-sm" style={{ display: 'block' }}>Daily Carbon Budget (gCO2)</label>
                                    <input type="number" className="input" style={{ width: '200px' }} value={settings.dailyCarbonBudget || 50} onChange={(e) => updateSetting('dailyCarbonBudget', parseInt(e.target.value))} />
                                    <p className="text-secondary text-sm mt-sm">Get notified when daily carbon usage exceeds this limit</p>
                                </div>
                                <div>
                                    <label className="text-secondary text-sm mb-sm" style={{ display: 'block' }}>Weekly Carbon Budget (gCO2)</label>
                                    <input type="number" className="input" style={{ width: '200px' }} value={settings.weeklyCarbonBudget || 250} onChange={(e) => updateSetting('weeklyCarbonBudget', parseInt(e.target.value))} />
                                    <p className="text-secondary text-sm mt-sm">Get notified when weekly carbon usage exceeds this limit</p>
                                </div>
                            </div>
                        </div>
                    )}
